...
class BaseEWHSPC(BaseDtofSPC):
  ...
  def capture(self, phi_bar):
    """
    phi_bar：形状为 (Nr, Nc, N_tbins) 的张量，表示单脉冲下每个像素、每个原始时间箱的平均入射光子数
    输出：包含等宽直方图的字典 captured_data，键为 "ewh"，值为形状 (Nr, Nc, N_ewhbins) 的光子计数张量
    """
    ...
    """
    hist = 0  #  用于累积所有激光脉冲下，每个原始时间箱的光子检测总计数，初始化为 0

    if self.N_pulses>1 and not(self.fast_sim):
      # 逐脉冲模拟，循环 N_pulses 次，每次调用 sim_poisson_process(phi_bar)（单脉冲二值化采样），将结果累加到 hist
      # 模拟真实的逐脉冲发射 - 检测过程：每次脉冲发射后，检测每个时间箱是否有光子（二值化 0/1），累加 N_pulses 次得到总计数
      for cycles in tqdm(range(self.N_pulses)):
        hist += self.sim_poisson_process(phi_bar) 
    else:
        # 快速多脉冲模拟，输入是多脉冲累积的平均光子数（phi_bar 乘以 N_pulses），一次采样得到总计数
        hist = self.sim_poisson_process_multicycle(phi_bar*self.N_pulses)

    # 合并原始时间箱，生成等宽直方图
    if self.factor>1:
      # factor>1 需要合并
      # hist.view(...)：改变张量形状，从 (Nr, Nc, N_tbins) 变为 (Nr, Nc, N_ewhbins, factor)
      # 将最后一个维度（原始时间箱）拆分为 “直方图 bin 数 × 合并因子”
      # .sum(dim=-1)：对最后一个维度factor 维度，即每个直方图 bin 对应的 factor 个原始时间箱求和，得到每个直方图 bin 的总光子数
      # 若 N_tbins=10，N_ewhbins=5，则 factor=2。hist 的形状为 (Nr, Nc, 10)，view 后变为 (Nr, Nc, 5, 2)，
      # sum(dim=-1) 后变为 (Nr, Nc, 5)，第 1 个直方图 bin 是原始时间箱 0+1 的和，第 2 个是 2+3 的和，以此类推
